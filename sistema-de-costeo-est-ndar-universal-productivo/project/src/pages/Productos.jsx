// CosteoUniversal — Productos (Industrial)
const { useState, useRef, useEffect } = React;

const PRODUCTOS_DATA = [
  { id: 'p1', nombre: 'Queso fresco 500g',  sku: 'QF-001', unidad: 'u', desc: 'Queso fresco artesanal empacado al vacío', costo: 38.70, precio: 55.00, margen: 30, ultimaFicha: 2,    activo: true  },
  { id: 'p2', nombre: 'Yogur natural 1L',   sku: 'YN-002', unidad: 'L', desc: 'Yogur natural sin azúcar añadida',         costo: 22.40, precio: 34.00, margen: 30, ultimaFicha: 1440, activo: true  },
  { id: 'p3', nombre: 'Mantequilla 200g',   sku: 'MT-003', unidad: 'u', desc: 'Mantequilla sin sal, barra 200g',          costo: 18.90, precio: 28.50, margen: 30, ultimaFicha: 4320, activo: true  },
  { id: 'p4', nombre: 'Requesón 300g',      sku: 'RQ-004', unidad: 'u', desc: 'Requesón fresco en recipiente plástico',   costo: 14.20, precio: 22.00, margen: 30, ultimaFicha: 7200, activo: true  },
  { id: 'p5', nombre: 'Queso maduro 1kg',   sku: 'QM-005', unidad: 'kg',desc: 'Queso semi-maduro, curación 30 días',      costo: 89.50, precio: 130.00,margen: 30, ultimaFicha: null, activo: true  },
  { id: 'p6', nombre: 'Crema de leche 1L',  sku: 'CL-006', unidad: 'L', desc: 'Crema entera pasteurizada',               costo: 31.00, precio: 45.00, margen: 30, ultimaFicha: null, activo: false },
];

const UNIDADES_OPT = ['u', 'kg', 'L', 'g', 'ml', 'docena', 'caja'];

const fichaStatus = mins => {
  if (mins === null) return { label: 'Sin ficha',      color: 'var(--text-tertiary)' };
  if (mins < 10080)  return { label: 'Ficha reciente', color: 'var(--accent-success)' };  // < 7 días
  if (mins < 43200)  return { label: 'Ficha antigua',  color: 'var(--accent-warning)' };  // < 30 días
  return                    { label: 'Desactualizada', color: 'var(--accent-danger)' };
};
...
const formatMinutes = mins => {
  ...
  return `hace ${Math.floor(mins/10080)}sem`;
};
